...
-- function DrawCross(renderer, x, y)
--     local red = Vector.Create(1, 0, 0, 1)
--     local size = 10
--     renderer:DrawLine2d(x, y - size, x, y + size, red)
--     renderer:DrawLine2d(x - size, y, x + size, y, red)
-- end
...
function CombatState:Render(renderer)
    ...
    -- For testing layout position
    -- local partyPos = CombatState.Layout['enemy'][5]
    -- for k, v in pairs(partyPos) do
    --     local x = v:X() * System.ScreenWidth()
    --     local y = v:Y() * System.ScreenHeight()
    --     DrawCross(renderer, x, y)
    -- end
