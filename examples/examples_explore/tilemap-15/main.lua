...
function PointToTile(x, y, tileSize, left, top, mapWidth, mapHeight)
    ...
    -- Tiles are render from the center so we adjust for this.
    x = x + tileSize / 2
    y = y - tileSize / 2
...
function update()
    ...
    local tileX, tileY = PointToTile(pos:X(), pos:Y(), gTileWidth, gLeft, gTop, gMapWidth, gMapHeight)
    ...
    -- Draw the mouse pointer as a red 2x2 pixel dot
    gRenderer:DrawRect2d(pos:X(), pos:Y(), pos:X() + 2, pos:Y() + 2,
                         Vector.Create(1, 0, 0, 1))

    -- Beneath the pointer display which tile we're intersecting.
    print(tileX, tileY)
    gRenderer:DrawText2d(pos:X(), pos:Y() - 30, string.format("X:%d,Y:%d", tileX, tileY))
end